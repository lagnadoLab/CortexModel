...
def simulate_step(tt, init, params):
    ...
    dd = np.vstack((sol.t, sol.y[0], sol.y[1], sol.y[2], sol.y[3])).T
    sim = pd.DataFrame(dd, columns=['t', 'f_e', 'f_pv', 'f_sst', 'f_vip'])
...
def residual_step(params, tt, init, data_pc, data_pv, data_sst, data_vip,
    ...
    pc_r = (np.float32(np.array(model['f_e'].values - data_pc))
            * weights_pc / pc_all_err_new).ravel()  # /pc_all_err_new

    pv_r = (np.float32(np.array(model['f_pv'].values - data_pv))
            * weights_pv / pv_err_new).ravel()  # /pv_err_new

    sst_r = (np.float32(np.array(model['f_sst'].values - data_sst))
             * weights_sst / sst_err_new).ravel()  # /sst_err_new

    vip_r = (np.float32(np.array(model['f_vip'].values - data_vip))
             * weights_vip / vip_err_new).ravel()  # /vip_err_new
