...
param_val = int(os.environ.get("PARAM_VAL", 1)) # Row id for getting parameters from dataframe (df_existing). 1 - default value to set if PARAM_VAL is not set.
print("Test param_val", param_val)

all_fits = pd.read_csv("good_fits_local_locomotion.csv")
iter_param = int(all_fits.iloc[param_val]["ID"])
...
pc_opto_pv_arch = np.loadtxt(DATA_DIR/'QA_Nor_AvgSess_PVArChT1_Opto.txt')
# PC during PV Chr opto
pc_opto_pv_chr = np.loadtxt(DATA_DIR/'QA_Nor_AvgSess_PVChr1_Opto.txt')
# PC during SST Arch opto
pc_opto_sst_arch = np.loadtxt(DATA_DIR/'QA_Nor_AvgSess_SSTArchT1_Opto.txt')
# PC during SST Chr opto
pc_opto_sst_chr = np.loadtxt(DATA_DIR/'QA_Nor_AvgSess_SSTChr1_Opto.txt')
...
# Select all parameter for each solution

df_ID = all_fits[all_fits["ID"]==param_val]
...
log_text(f"Result for Parameter Set {param_val}\n{rmse_full_txt}\n{rmse_full_stim_txt}")
