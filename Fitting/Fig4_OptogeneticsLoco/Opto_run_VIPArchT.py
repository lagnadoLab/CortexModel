...
good_fits = pd.read_csv("good_fits.csv")
...
def model_step(t,
    ...
    if (t>=5): #and (t<=15):
        ...
        if opto_type == "VIP_Arch":
            ...
            df_v_odt = ((q * (ff_v_o/ opto_param) ** power) - f_v_o) / tau_3
...
# PC during VIP Arch opto
pc_opto_vip_arch = np.loadtxt(r'SST_VIPArch_Opto.txt')
...
for ind, time in np.ndenumerate(t_exp_1):
    if (time >= 5) and (time <= 25):
        ...
        pc_opto_vip_arch_new.append(pc_opto_vip_arch[ind])
...
pc_opto_vip_arch_new.insert(0, pc_opto_vip_arch_new[0])
...
all_fits = pd.read_csv("combined_reduced.csv")
...
opto_type = "VIP_Arch"  # "PV_Arch", "PV_Chr", "SST_Arch", "SST_Chr"
...
data_pc_opto_vip_arch = np.float32(np.array(pc_opto_vip_arch_new))
...
data_pc_opto = data_pc_opto_vip_arch
opto_type = "VIP_Arch"
