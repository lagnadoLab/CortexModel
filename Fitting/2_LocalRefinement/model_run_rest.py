...
pc_all_err = np.loadtxt(DATA_DIR/'QA_SEM_Sess1_PC_NoLoco.txt')
...
pv_err = np.loadtxt(DATA_DIR/'QA_SEM_Sess1_PV_NoLoco.txt')

for i, v in np.ndenumerate(pv_err):
    if v == 0:
        pv_err[i] = stats.mode([i for i in pv_err if i > 0]).mode

# SST_average (whole population)
sst = np.loadtxt(DATA_DIR/'QA_Avg_Sess1_SST_NoLoco.txt')
# SST_average_standard_error
sst_err = np.loadtxt(DATA_DIR/'QA_SEM_Sess1_SST_NoLoco.txt')

for i, v in np.ndenumerate(sst_err):
    if v == 0:
        sst_err[i] = stats.mode([i for i in sst_err if i > 0]).mode

# VIP_average (whole population)
vip = np.loadtxt(DATA_DIR/'QA_Avg_Sess1_VIPPos_NoLoco.txt')
# VIP_average_standard_error
vip_err = np.loadtxt(DATA_DIR/'QA_SEM_Sess1_VIP_NoLoco.txt')

for i, v in np.ndenumerate(vip_err):
    if v == 0:
        vip_err[i] = stats.mode([i for i in vip_err if i > 0]).mode


input_file = "sobol_local_rest.csv"
...
init = np.float32(np.array([0.055, 0.011, 0.017, 0.308]))
...
result_nelder = minimize(residual_step, params, method='nelder',
                         args=(t_exp, init, data_pc_all,
                               data_pv, data_sst, data_vip,
                               pc_all_err_new, pv_err_new,
                               sst_err_new, vip_err_new),
