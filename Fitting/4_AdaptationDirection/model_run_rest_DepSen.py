...
from model_functions_DepSen import *
import argparse
...
if args.pc_type == "sensitizers":
    pc_sen = np.loadtxt(DATA_DIR/'QA_Norx2_Sen_Avg1_NoLoco.txt')
    pc_sen_err = np.loadtxt(DATA_DIR/'QA_Norx2_Sen_SEM_NoLoco.txt')
elif args.pc_type == "intermediate":
    pc_sen = np.loadtxt(DATA_DIR/'QA_Norx2_NA_Avg1_NoLoco.txt')
    pc_sen_err = np.loadtxt(DATA_DIR/'QA_Norx2_NA_SEM_NoLoco.txt')
elif args.pc_type == "depressors":
    pc_sen = np.loadtxt(DATA_DIR/'QA_Norx2_Dep_Avg1_NoLoco.txt')
    pc_sen_err = np.loadtxt(DATA_DIR/'QA_Norx2_Dep_SEM_NoLoco.txt')
...
pc_all = np.loadtxt(DATA_DIR/'QA_Avg_Sess1_PC_NoLoco.txt')

# Experimental time calculation
t_exp_1 = exp_time(start=0.0, step=dt, count=len(pc_all))

# PV_average (whole population)
pv = np.loadtxt(DATA_DIR/'QA_Avg_Sess1_PV_NoLoco.txt')

# SST_average (whole population)
sst = np.loadtxt(DATA_DIR/'QA_Avg_Sess1_SST_NoLoco.txt')

# VIP_average (whole population)
vip = np.loadtxt(DATA_DIR/'QA_Avg_Sess1_VIPPos_NoLoco.txt')

# Load good fits from central model fit

input_file = "good_fits_local_rest.csv"

if os.path.exists(input_file):
    df_all = pd.read_csv(input_file)
    last_id = df_all["ID"].max()
else:
    print("Good fits file didn't exist. Created a new one.")
    df_all = pd.DataFrame()
    last_id = 0

df_existing = df_all[["w_0", "w_1", "w_2", "w_3", "w_4", "w_5", "w_6",
