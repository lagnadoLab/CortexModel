...
if args.pc_type == "sensitizers":
    pc_sen = np.loadtxt(DATA_DIR/'QA_Norx2_Sen_Avg1_Loco.txt')
    pc_sen_err = np.loadtxt(DATA_DIR/'QA_Norx2_Sen_SEM1_Loco.txt')
elif args.pc_type == "intermediate":
    pc_sen = np.loadtxt(DATA_DIR/'QA_Norx2_NA_Avg1_Loco.txt')
    pc_sen_err = np.loadtxt(DATA_DIR/'QA_Norx2_NA_SEM1_Loco.txt')
elif args.pc_type == "depressors":
    pc_sen = np.loadtxt(DATA_DIR/'QA_Norx2_Dep_Avg1_Loco.txt')
    pc_sen_err = np.loadtxt(DATA_DIR/'QA_Norx2_Dep_SEM1_Loco.txt')
...
input_file = "good_fits_opto_locomotion.csv"
...
#df_wide.to_csv(f"results_locomotion/output_ID_{int(param_val)}.csv", index=False)
...
os.makedirs("results_locomotion", exist_ok=True)
df_result.to_csv(f"results_locomotion/output_ID_{int(param_val)}.csv", index=False)
