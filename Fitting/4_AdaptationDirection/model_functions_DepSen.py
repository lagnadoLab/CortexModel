...
def model_step(t,
               ...
               s_start, k, i_d_0, w_d_0, w_d_1, w_d_2, w_d_3, w_d_4, w_d_5
               ):
    """
    Model basis:
    ------------------------------------
    ...
    ff_e_d = (min(max((i_d_0 
                        + w_d_0 * f_e 
                        + w_d_1 * expon(t, ampl = ampl, base = base, decay = decay,
                                         delay = delay_1, decay_ff = decay_ff, s_start = s_start, k = k) 
                        + w_d_2 * Sigm(t, ampl = ampl_1, rate = r_1, delay = delay_2, decay_s = decay_s, base = base_sigm) 
                        + w_d_3 * Step(t, k = 0, delay = delay_3, decay_f = decay_f) 
                        - w_d_4 * f_p 
                        - w_d_5 * f_s), threshold), 25))
    ...
    dydt = [df_edt, df_pdt, df_sdt, df_vdt, df_e_ddt]
...
def simulate_step(tt, init, params):
    ...
    sim = pd.DataFrame(dd, columns=['t', 'f_e', 'f_pv', 'f_sst', 'f_vip', 'f_e_d'])

    return sim
    
    
def smooth3(x):
    """Equivalent to Igor Pro's 'Smooth 3' (one pass)."""
    kernel = np.array([1, 2, 1], dtype=float) / 4.0
    smoothtrace = np.convolve(x, kernel, mode='same')
    smoothtrace = np.convolve(smoothtrace, kernel, mode='same')
    smoothtrace = np.convolve(smoothtrace, kernel, mode='same')  # third pass
    return smoothtrace

def residual_ad(params, tt, init, data_pc_sen, pc_err):
    """
    ...
    tt (running)        - current timepoint;
    init (fixed)        - intial conditions for differential equations;
    data_pc_sen         - PC Dep/NA/Sen experimental data trace;
    ...
    model_trace = np.array(model['f_e_d'].values)
    
    # ---- smooth both model and data (Igor-style Smooth 5) ----
    model_trace_smth = smooth3(model_trace)
    data_pc_sen_smth = smooth3(data_pc_sen)
    
    pc_d_r = ((model_trace_smth - data_pc_sen_smth) * weights_pc / pc_err).ravel()
    
    return pc_d_r


def RMSE_full(params, init, data_pc, data_pv, data_sst, data_vip, t_exp):
    """
    Root:Mean Square Error (RMSE) calculation:
    ------------------------------------
    Returns RMSE value for full fit of 1 averages of Dep/NA/Sen PCs.
    ------------------------------------
    Parameters:
    ...
    for i in range(len(data_pc)):
        ...
        sum += ((model['f_e'].values[i] - data_pc[i]) ** 2
                ...
                + (model['f_vip'].values[i] - data_vip[i]) ** 2)
...
def RMSE_full_stim(params, init, data_pc, data_pv, data_sst, data_vip, t_exp):
    """
    Root:Mean Square Error (RMSE) calculation:
    ------------------------------------
